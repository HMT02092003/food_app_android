...
public class HomeActivity extends AppCompatActivity implements CategoryHomeAdapter.OnCategoryClickListener {
    ...
    private void setupListeners() {
        ...
        // Xử lý nút "Xem thêm" ở phần Món ăn nổi bật
        featuredFoodSeeMoreButton.setOnClickListener(v -> {
            Intent intent = new Intent(HomeActivity.this, ListFoodsActivity.class);
            intent.putExtra("CategoryName", "Món ăn nổi bật");
            intent.putExtra("CategoryId", 0); // 0 để lấy tất cả món ăn
            intent.putExtra("isSearch", false);
            startActivity(intent);
        });

        // Xử lý nút "Xem thêm" ở phần Danh sách món ăn
        Button foodListSeeMoreButton = findViewById(R.id.foodListSeeMoreButton);
        if (foodListSeeMoreButton != null) {
            foodListSeeMoreButton.setOnClickListener(v -> {
        ...
        EditText searchEdt = findViewById(R.id.searchEdt);
        if (searchBtn != null && searchEdt != null) {
    ...
    private void navigateToDetailActivity(FoodModel food) {
        ...
        detailIntent.putExtra("foodRecipe", food.getRecipe());
